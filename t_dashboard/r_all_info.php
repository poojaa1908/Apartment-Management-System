...
if(isset($_GET['fid']) && $_GET['fid'] != ''){
  $sq=mysqli_query($conn,"SELECT * FROM tbl_pay where f_id='".$_GET['fid']."'");
  $row = mysqli_fetch_array($sq);
    ...
    $s6=$row['rent'];
    $s7=$row['water_bill'];
    $s8=$row['electric_bill'];
    $s9=$row['gas_bill'];
    $s10=$row['security_bill'];
    $s11=$row['utility_bill'];
    $s12=$row['other_bill'];
    $s13=$row['total_rent'];
    $s14=$row['issue_date'];
    $s15=$row['branch_id'];

  $sql = "INSERT INTO tbl_add_fair(type,floor_no,unit_no,rid,month_id,xyear,rent,water_bill,electric_bill,gas_bill,security_bill,utility_bill,other_bill,total_rent,issue_date,branch_id) values('Rented','$s1','$s2','$s3','$s4','$s5','$s6','$s7','$s8','$s9','$s10','$s11','$s12','$s13','$s14','$s15')";
	$sqlx= "DELETE FROM `tbl_pay` WHERE f_id = ".$_GET['fid'];
	mysqli_query($conn,$sqlx); 
  mysqli_query($conn,$sql); 
	mysqli_close($conn); 
  $url = WEB_URL . 't_dashboard/tenant_details.php';
	header("Location: $url");
	$delinfo = 'block';
}
?>
...
<!-- /.row -->
<div align="center"><a class="btn btn-primary btn_save" onClick="deleteComplain(<?php echo $row['f_id']; ?>);" href="javascript:;"><?php echo "PAY";?></a></div><br></br>
...
<script>
  function deleteComplain(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL; ?>t_dashboard/r_all_info.php?fid=' + Id;
	}
  }
  </script>
