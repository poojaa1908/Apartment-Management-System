...
$addinfo = 'none';
$msg = "";
$assign = $_SESSION['objLogin']['e_name'];
    ...
    <div class="box box-info">
      ...
      <div class="box-body">
        <table class="table sakotable table-bordered table-striped dt-responsive">
          ...
          <tbody>
            <?php
				$result = mysqli_query($conn, "SELECT * FROM tbl_add_complain where assign = '$assign' AND branch_id = " . (int)$_SESSION['objLogin']['branch_id'] . " order by complain_id");
				while($row = mysqli_fetch_array($result)){?>
            <tr>
              <td><?php echo $row['c_date']; ?></td>
              <td><?php echo $row['c_title']; ?></td>
              <td><?php echo $row['c_description']; ?></td>
              <td><a class="btn btn-danger" data-toggle="tooltip" href="javascript:;" onClick="$('#nurse_view_<?php echo $row['complain_id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>"><i class="fa fa-eye"></i></a> <a class="btn btn-success" data-toggle="tooltip" onClick="deleteComplain(<?php echo $row['complain_id']; ?>);" href="javascript:;" data-original-title="<?php echo "Done";?>"><i class="fa fa-trash-o"></i></a>
...
function deleteComplain(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL; ?>e_dashboard/ecomplainlist.php?id=' + Id;
	}
  }
